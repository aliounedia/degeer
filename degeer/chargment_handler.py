...
class DejaReaboHandler(object):
                  ...
                  try:
                     ...
                     for root_, dir, files in os.walk(self.root):
                          for f in  files:
                              ...
                              # supprime le fichier que ftp_client.py a depose
                              ...
                              for l in f.readlines():
                                  ...
                                  if len(row)==18:
                                     ...
                                     row.extend([f_name.split('/')[-1], TODAY, main_fichier, '00221'+ main_tel])
                                     rows.append(row)
                                     row= []                              
                              f.close()
      ...
      def handle_afr(self):
            '''
            Starting to handle  root dir , Concerne tout ce qui n'est pas senegal
            -CAMEROUN
            -CONGO
            -RDC
            -BENIN
                                ...
                                for row in rd:
                                     ...
                                     if 'AFR_AS_AAE1M_RECUR' in main_fichier:
                                          if  row[22] in  ('BURKINA FASO'):
                                                main_fichier = 'CANAL_OA_ARRIVANT_A_ECHEANCE_BURKINA_1MC_20120201'
                                          elif  row[22] in  ('CAMEROUN'):
                                                main_fichier = 'CANAL_OA_ARRIVANT_A_ECHEANCE_CAMEROUN_1MC_20120201'
                                          elif  row[22] in  ('CONGO'):
                                                main_fichier = 'CANAL_OA_ARRIVANT_A_ECHEANCE_CONGO_1MC_20120201'
                                          elif  row[22] in  ('REP.DEMOCRATIQUE DU CONGO'):
                                                main_fichier = 'CANAL_OA_ARRIVANT_A_ECHEANCE_RDC_1MC_20120201'
                                          else: 
                                                main_fichier = 'CANAL_OA_ARRIVANT_A_ECHEANCE_AFR_1MC_20120201' 
                                     if 'AFR_AS_EF_RECUR'  in main_fichier:
                                          if  row[22] in  ('BURKINA FASO'):
                                                main_fichier = 'CANAL_PA_ECHUS_FRAIS_BURKINA_20120201'
                                          elif  row[22] in  ('CAMEROUN'):
                                                main_fichier = 'CANAL_PA_ECHUS_FRAIS_CAMEROUN_20120201'
                                          elif  row[22] in  ('CONGO'):
                                                main_fichier = 'CANAL_PA_ECHUS_FRAIS_CONGO_20120201'
                                          elif  row[22] in  ('REP.DEMOCRATIQUE DU CONGO'):
                                                main_fichier = 'CANAL_PA_ECHUS_FRAIS_RDC_20120201'
                                          else: 
                                                main_fichier = 'CANAL_PA_ECHUS_FRAIS_AFR_20120201'   
                                     if 'AFR_AS_RECRU_RECUR' in main_fichier:
                                                main_fichier = 'CANAL_PA_WELCOME_CALL_20120201'
                                                 
                                     row.extend([f_name.split('/')[-1], TODAY, main_fichier,  main_tel])
                                     rows.append(row)
      ...
      def flush(self):
          ''' Return un dictionary that contain name and file data'''
      ...
      def update_main_from_file_senegal(self):
          ...
          try:
                  ...
                  print  '\n'
                  db = DB()
                  db.excute_many_for_senegal(query, self.map)
          
          except:
              # Bizzare peu etre un probleme avec la base de
              # donnee 
              import sys
              print sys.exc_info()
          else:
              ...
              while True:
                    try:
                        ...
                        subject="[PCCI] Chargement fichier du %s """%TODAY     
                        ...
                        # L'email est partie tout va bien,super
                        break
      def update_main_from_file_afr(self):
          ...
          try:
                  ...
                  print  '\n'
                  db = DB()
                  db.excute_many_for_afr(query, self.map)
      ...
      def main_fichier(self):
         ...
         #pour alimenter le corps du mail
         return '</br>'.join([ '%s ** %s'%(str(k),str(v)) for (k, v) in out.items() ])
       

# pour les testes
def test_senegal():
    d =DejaReaboHandler(sql_main ='MAIN_INSERT_SENEGAL.sql' ,root ='SEN_A_PCCI\TMK', sql_deja_rea =None)
    d.handle_senegal()
def test_afr():
...
# A lancer comme un script 
if __name__=='__main__':
   # chaque thread est lance avec les bons parametre c'oublie
   # surtout pas (le SENEGAL et L'AFRIQUE)
   import  time
   import threading
   t1=  threading.Thread(target = DejaReaboHandler(sql_main ='MAIN_INSERT_SENEGAL.sql' ,
                                                   sql_deja_rea =None , root ='SEN_A_PCCI\TMK').handle_senegal)
   t2=  threading.Thread(target  = DejaReaboHandler(sql_main ='MAIN_INSERT_AFR.sql' ,
                                                    root ='COSA_A_PCCI\TMK', sql_deja_rea =None).handle_afr)
